...
if (process.env.YMZ_BODY1 && process.env.YMZ_BODY1.split('\n').length > 0) {
 ...
 if (process.env.YMZ_TXBODY && process.env.YMZ_TXBODY.split('\n').length > 0) {
   ymztxbody = process.env.YMZ_TXBODY.split('\n');
  } else  {
   ymztxbody = process.env.YMZ_TXBODY.split()
  };
   ...
   Object.keys(ymzhd).forEach((item) => {
    ...
    Object.keys(ymztxbody).forEach((item) => {
        if (ymztxbody[item]) {
          ymztxbodyArr.push(ymztxbody[item])
        }
    });  
   ...
   // ymzurlArr.push($.getdata('ymzurl'))
    ...
    ymztxbodyArr.push($.getdata('ymztxbody'))
 ...
 for (let i = 2; i <= accountcount; i++) {
   // ymzurlArr.push($.getdata(`ymzurl${i}`))
    ...
    ymztxbodyArr.push($.getdata(`ymztxbody${i}`))
  }
}
